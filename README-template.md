...
```bash
dotnet run --project src/Host/MIT.Starter.AppHost
```
...
```bash
dotnet run --project src/Host/MIT.Starter.Api      # needs external Postgres + Redis
```
...
src/
  ...
  Host/
    MIT.Starter.Api/                    API composition root
    MIT.Starter.AppHost/                .NET Aspire orchestrator
    MIT.Starter.DbMigrator/             One-shot migrate / seed runner
    MIT.Starter.Migrations.PostgreSQL/  EF Core migrations
...
- [ ] **Mail** — configure SMTP / SendGrid under `MailOptions` in
      `src/Host/MIT.Starter.Api/appsettings.json`.
...
```bash
dotnet test src/MIT.Starter.slnx       # integration tests require Docker
```
