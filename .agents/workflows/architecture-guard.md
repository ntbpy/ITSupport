...
### 3. Build clean
```bash
dotnet build src/MIT.Starter.slnx 2>&1 | grep -E "warning|error"   # expect none (TreatWarningsAsErrors)
```

### 4. Module boundary heuristic
```bash
grep -rn "using MIT.Modules\." src/Modules --include="*.cs" | grep -v "\.Contracts"
```
...
If a new `*Module` was added, confirm it appears in **all four**: Mediator `o.Assemblies` (Contracts marker **and** module type) + `moduleAssemblies` array, in **both** `MIT.Starter.Api/Program.cs` and `MIT.Starter.DbMigrator/Program.cs`.
```bash
grep -rn "{New}Module\|{New}ContractsMarker" src/Host/MIT.Starter.Api/Program.cs src/Host/MIT.Starter.DbMigrator/Program.cs
```
