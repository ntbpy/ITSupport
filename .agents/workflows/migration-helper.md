...
- All migrations live in **one** project, `src/Host/MIT.Starter.Migrations.PostgreSQL`, foldered **per module/context** (`Catalog/`, `Identity/`, …), each with its own `{X}DbContextModelSnapshot`.
- Startup project is `src/Host/MIT.Starter.Api`. Always pass `--context {X}DbContext` and `--output-dir {X}`.
...
```bash
dotnet run --project src/Host/MIT.Starter.DbMigrator -- list-pending   # preview
...
| "Build failed" | `dotnet build src/MIT.Starter.slnx` first |
