...
5. **⚠️ Register in ALL FOUR places** — Mediator `o.Assemblies` (Contracts marker **and** module type) + `moduleAssemblies` array, in **both** `MIT.Starter.Api/Program.cs` **and** `MIT.Starter.DbMigrator/Program.cs`. Add to `.slnx`; reference the runtime project from Api, DbMigrator, and the Migrations project.
...
dotnet run --project src/Host/MIT.Starter.DbMigrator -- list-pending   # new context shows up
```
