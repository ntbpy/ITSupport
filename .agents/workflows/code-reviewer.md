...
dotnet build src/MIT.Starter.slnx 2>&1 | grep -E "warning|error"               # 0 expected
```
...
### Verification
dotnet build src/MIT.Starter.slnx   → expect 0 warnings
dotnet test src/MIT.Starter.slnx    (integration tests need Docker)
```
