...
Login `POST /api/v1/identity/token/issue` with header `X-FSH-App: "admin"|"dashboard"`. **localStorage keys are namespaced per app** (`MIT.admin.*` / `MIT.dashboard.*`) so both run side-by-side. Permission *source* differs per app — see the app files.
...
- **JWT seeding:** `seedAuthedSession(page, TEST_USER)` builds a fake JWT and `addInitScript`-writes `MIT.{app}.*` to localStorage before React boots (server isn't called, so signature is junk).
