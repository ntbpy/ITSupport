...
- **Port** 5173 · dev proxy target `http://localhost:5030` (HTTP) · localStorage prefix `MIT.admin.*` · login header `X-FSH-App: admin`.
...
- The JWT carries only role names. Admin fetches the permission set separately: `GET /api/v1/identity/permissions` (`getMyPermissions`), cached under `MIT.admin.permissions`.
...
- Playwright: `seedAuthedSession` here also pre-seeds `MIT.admin.permissions` so `RouteGuard` passes on first paint.
