...
```csharp
[assembly: FshModule(typeof(MIT.Modules.Identity.IdentityModule), 1)]   // above the namespace
...
|---|---|---|
| Mediator `o.Assemblies` (two markers: Contracts type **and** module type) | `src/Host/MIT.Starter.Api/Program.cs` | Handlers silently undiscovered |
| `moduleAssemblies` array | `src/Host/MIT.Starter.Api/Program.cs` | Module never loaded |
| Mediator assemblies (same pair) | `src/Host/MIT.Starter.DbMigrator/Program.cs` | Migrate/seed misses the module |
| module assemblies array | `src/Host/MIT.Starter.DbMigrator/Program.cs` | Migrate/seed misses the module |
...
- `appsettings.json` (+ `.Development`/`.Production`) live in `src/Host/MIT.Starter.Api/`. DbMigrator links the same files.
