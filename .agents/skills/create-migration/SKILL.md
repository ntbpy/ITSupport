...
All migrations live in **one** project — `src/Host/MIT.Starter.Migrations.PostgreSQL` — but are foldered
...
dotnet ef migrations add {MigrationName} \
  --project src/Host/MIT.Starter.Migrations.PostgreSQL \
  --startup-project src/Host/MIT.Starter.Api \
  --context {X}DbContext \
  --output-dir {X}
```
...
dotnet run --project src/Host/MIT.Starter.DbMigrator -- list-pending   # to preview first
```
