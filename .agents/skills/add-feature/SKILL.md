...
```csharp
namespace MIT.Modules.{X}.Contracts.v1.{Area};
...
(`MIT.Framework.Shared.Persistence`) — see `query-patterns`.
...
Throw `NotFoundException` / `CustomException(msg, errors, HttpStatusCode)` (`MIT.Framework.Core.Exceptions`) — the global handler maps them to ProblemDetails.
...
```bash
dotnet build src/MIT.Starter.slnx          # 0 warnings (TreatWarningsAsErrors)
