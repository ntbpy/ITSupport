...
description: CQRS interface reference for MIT. This project uses the Mediator source generator, NOT MediatR. Reference when implementing commands, queries, and handlers.
...
list **plus** an entry in the `moduleAssemblies` array — in **both** `MIT.Starter.Api/Program.cs` **and**
`MIT.Starter.DbMigrator/Program.cs`:
...
builder.Services.AddMediator(o =>
{
    ...
    o.Assemblies = [
        /* … */
        typeof(MIT.Modules.{X}.Contracts.{X}ContractsMarker),   // Contracts assembly
        typeof(MIT.Modules.{X}.{X}Module)];                     // runtime assembly
});
```
