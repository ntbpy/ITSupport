...
if ($BuildApi) {
  ...
  dotnet publish "$RepoRoot/src/Host/MIT.Starter.Api/MIT.Starter.Api.csproj" `
  ...
  dotnet publish "$RepoRoot/src/Host/MIT.Starter.DbMigrator/MIT.Starter.DbMigrator.csproj" `
    -c Release -r linux-x64 `
    /t:PublishContainer `
    -p:ContainerRegistry="$registryHost" `
