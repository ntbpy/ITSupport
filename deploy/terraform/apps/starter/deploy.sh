...
if [[ "$BUILD_API" == true ]]; then
  ...
  dotnet publish "$REPO_ROOT/src/Host/MIT.Starter.Api/MIT.Starter.Api.csproj" \
  ...
  dotnet publish "$REPO_ROOT/src/Host/MIT.Starter.DbMigrator/MIT.Starter.DbMigrator.csproj" \
    -c Release -r linux-x64 \
    /t:PublishContainer \
    -p:ContainerRegistry="$REGISTRY_HOST" \
