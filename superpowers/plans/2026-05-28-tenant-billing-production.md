...
**Phase A gate:** `dotnet build src/MIT.Starter.slnx` clean; `dotnet test` Billing+Multitenancy+Integration green.
...
- [ ] Migration in `MIT.Starter.Migrations.PostgreSQL` (tenant schema). Full-build before `migrations add`.
- [ ] Commit.
...
## Final
- [ ] `dotnet build src/MIT.Starter.slnx` (TreatWarningsAsErrors) + full `dotnet test` (Docker up).
