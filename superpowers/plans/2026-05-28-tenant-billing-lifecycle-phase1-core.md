...
**Migrations**
- `src/Host/MIT.Starter.Migrations.PostgreSQL/<Billing folder>` — one migration: plan columns +
...
namespace MIT.Modules.Billing.Tests.Domain;
...
- [ ] **Step 5:** Build. Run: `dotnet build src/MIT.Starter.slnx`. Expected: PASS.
...
**Files:** `src/Host/MIT.Starter.Migrations.PostgreSQL/<Billing folder>/`

- [ ] **Step 1:** Full build first (avoids the `migrations add` snapshot footgun):
  `dotnet build src/MIT.Starter.slnx`.
- [ ] **Step 2:** Add migration (use the repo's EF migration convention / `create-migration` skill —
  ...
  `dotnet ef migrations add BillingPlanIntervalAndInvoicePurpose -c BillingDbContext -p src/Host/MIT.Starter.Migrations.PostgreSQL -s src/Host/MIT.Starter.Api -o <Billing migrations folder>`
...
- [ ] **Step 4:** Apply to a scratch DB: `dotnet run --project src/Host/MIT.Starter.DbMigrator -- apply`.
...
- [ ] **Step 1:** `dotnet build src/MIT.Starter.slnx` (warnings = errors) → PASS.
- [ ] **Step 2:** `dotnet test src/MIT.Starter.slnx` (Docker up for integration) → PASS.
