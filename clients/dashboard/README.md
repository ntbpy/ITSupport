...
├── auth/                 # JWT-backed auth (own localStorage prefix: MIT.dashboard.*)
├── components/
...
Identical to the admin app: JWT in `localStorage`, `Authorization: Bearer` + `tenant` headers, single-flight refresh on 401 via `POST /api/v1/identity/token/refresh`. Keys are namespaced `MIT.dashboard.*` so both apps can run side-by-side without clobbering each other's session.
