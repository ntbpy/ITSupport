...
| `src/Host/MIT.Starter.Api` | Composition-root Web API host. |
| `src/Host/MIT.Starter.AppHost` | .NET Aspire orchestrator (Postgres, Redis, MinIO, migrator, API, **both React apps**). |
| `src/Host/MIT.Starter.DbMigrator` | One-shot migrate/seed runner. DB is **not** migrated at API startup. |
| `src/Host/MIT.Starter.Migrations.PostgreSQL` | All EF migrations, organized per-module by folder. |
...
dotnet run --project src/Host/MIT.Starter.AppHost   # one-time: npm install in clients/admin & clients/dashboard

dotnet build src/MIT.Starter.slnx                   # build backend
dotnet run --project src/Host/MIT.Starter.Api       # API only → https://localhost:7030 (/scalar)
dotnet test src/MIT.Starter.slnx                    # tests — integration tests REQUIRE Docker
...
```bash
dotnet run --project src/Host/MIT.Starter.DbMigrator -- apply [--seed]
dotnet run --project src/Host/MIT.Starter.DbMigrator -- list-pending
```
