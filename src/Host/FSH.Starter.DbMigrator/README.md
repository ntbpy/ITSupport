...
dotnet run --project src/Host/MIT.Starter.DbMigrator -- apply

# Apply only to one tenant.
dotnet run --project src/Host/MIT.Starter.DbMigrator -- apply --tenant root

# Apply only the tenant catalog (no per-tenant pass).
dotnet run --project src/Host/MIT.Starter.DbMigrator -- apply --catalog-only
...
dotnet run --project src/Host/MIT.Starter.DbMigrator -- seed
...
DOTNET_ENVIRONMENT=Development \
  dotnet run --project src/Host/MIT.Starter.DbMigrator -- seed-demo
```
...
as `MIT.Starter.Api` (both files are linked into the project so they
...
| `DatabaseOptions__MigrationsAssembly`     | `MIT.Starter.Migrations.PostgreSQL`         |
...
```bash
dotnet publish src/Host/MIT.Starter.DbMigrator -c Release \
        ...
        - name: migrator
          ...
          env:
            ...
            - name: DatabaseOptions__MigrationsAssembly
              value: MIT.Starter.Migrations.PostgreSQL
```
...
- **Aspire**: `dotnet run --project src/Host/MIT.Starter.AppHost` —
...
- **Raw**: run the migrator once after pulling, before starting the
  ...
  # Then start the API
  dotnet run --project src/Host/MIT.Starter.Api
  ```
