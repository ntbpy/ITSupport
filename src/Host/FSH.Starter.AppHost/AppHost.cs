...
// Per-app prefix from the AppHost assembly name (MIT.Starter.AppHost -> fsh-starter); namespaces Docker volumes + resource names so multiple FSH apps don't clash.
...
var migrator = builder.AddProject<Projects.FSH_Starter_DbMigrator>($"{appPrefix}-db-migrator")
    ...
    .WithEnvironment("DatabaseOptions__MigrationsAssembly", "MIT.Starter.Migrations.PostgreSQL")
