...
- **Feature layering**: feature types under `Modules.*.Features.v{version}` are checked with NetArchTest to depend only on allowed layers (System/Microsoft, `MIT.Framework.*`, their module, and module contracts).
...
- Run all tests (including architecture tests): `dotnet test src/MIT.Starter.slnx`.
