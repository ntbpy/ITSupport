...
using MIT.Framework.Jobs.Services;
...
public sealed class FshWebApplicationFactory : WebApplicationFactory<Program>, IAsyncLifetime
{
    ...
    protected override void ConfigureWebHost(IWebHostBuilder builder)
    {
        ...
        builder.ConfigureAppConfiguration((_, config) =>
        {
            config.AddInMemoryCollection(new Dictionary<string, string?>
            {
                ...
                ["DatabaseOptions:MigrationsAssembly"] = "MIT.Starter.Migrations.PostgreSQL",
    ...
    private void RewireStorageForS3(IServiceCollection services)
    {
        var toRemove = services
            .Where(d => d.ServiceType == typeof(MIT.Framework.Storage.Services.IStorageService)
                     || d.ServiceType == typeof(MIT.Framework.Storage.Local.LocalStorageService)
                     || d.ServiceType == typeof(MIT.Framework.Storage.S3.S3StorageService)
                     || d.ServiceType == typeof(IAmazonS3))
            .ToList();
        foreach (var d in toRemove) services.Remove(d);

        services.Configure<MIT.Framework.Storage.S3.S3StorageOptions>(opts =>
        {
        ...
        services.AddTransient<MIT.Framework.Storage.S3.S3StorageService>();
        services.AddTransient<MIT.Framework.Storage.Services.IStorageService>(sp =>
            sp.GetRequiredService<MIT.Framework.Storage.S3.S3StorageService>());
    }
    ...
    private async Task ProvisionRootTenantAsync()
    {
        ...
        using (var scope = Services.CreateScope())
        {
            ...
            var syncer = scope.ServiceProvider.GetRequiredService<MIT.Modules.Identity.Authorization.RolePermissionSyncer>();
