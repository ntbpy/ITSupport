...
public sealed class TenantThemeTests : IAsyncLifetime
{
    ...
    public async Task UpdateTheme_Should_NotLeakAcrossTenants_When_RootOperatorTargetsTenantA()
    {
        ...
        clieMIT.DefaultRequestHeaders.Authorization = new("Bearer", rootToken.AccessToken);
        clieMIT.DefaultRequestHeaders.Add("tenant", _tenantB);
        var responseB = await clieMIT.GetAsync(ThemePath);
    ...
    public async Task GetTheme_Should_StayInOwnTenant_When_TenantBAdminSendsTenantAHeader()
    {
        ...
        using var clientB = _factory.CreateClient();
        clieMIT.DefaultRequestHeaders.Authorization = new("Bearer", tokenB.AccessToken);
        clieMIT.DefaultRequestHeaders.Add("tenant", _tenantA); // spoof attempt — must be ignored

        // Act
        var response = await clieMIT.GetAsync(ThemePath);
    ...
    public async Task UpdateTheme_Should_NotMutateTenantA_When_TenantBAdminSendsTenantAHeader()
    {
        ...
        {
            clieMIT.DefaultRequestHeaders.Authorization = new("Bearer", tokenB.AccessToken);
            clieMIT.DefaultRequestHeaders.Add("tenant", _tenantA); // spoof attempt
            var attack = await clieMIT.PutAsJsonAsync(ThemePath, ValidTheme(primary: "#000000"));
