...
public sealed class ObservableHybridCacheTests : IDisposable
{
    ...
    public ObservableHybridCacheTests()
    {
        ...
        _listener.SetMeasurementEventCallback<long>((instrument, measurement, tags, state) =>
        {
            ...
            {
                case "MIT.cache.hits": Interlocked.Add(ref _hits, measurement); break;
                case "MIT.cache.misses": Interlocked.Add(ref _misses, measurement); break;
                case "MIT.cache.invalidations": Interlocked.Add(ref _invalidations, measurement); break;
            }
        });
        _listener.SetMeasurementEventCallback<double>((instrument, measurement, tags, state) =>
        {
            if (instrument.Name == "MIT.cache.factory.duration")
            {
