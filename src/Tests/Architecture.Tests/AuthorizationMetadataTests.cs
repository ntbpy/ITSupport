...
public class AuthorizationMetadataTests
{
    ...
    private const string ExpectedNamespace = "MIT.Framework.Shared.Identity.Authorization";
    ...
    /// <summary>
    /// Loads every MIT.* assembly deployed alongside the tests so the duplicate sweep covers
    ...
    private static Assembly[] GetAllFshAssemblies()
    {
        ...
        foreach (var file in Directory.GetFiles(baseDir, "MIT.*.dll"))
        {
            try
            {
...
#pragma warning restore CA1031
        ...
        assemblies.ShouldNotBeEmpty(
            "No MIT.* assemblies were found in the test output directory; the duplicate sweep would be a no-op.");
